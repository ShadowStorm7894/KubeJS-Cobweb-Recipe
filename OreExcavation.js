ServerEvents.recipes(event => {
	...
	function CustomVeinDrillingFD(title, output, spacing, separation, salt, id, sizem, sizeM, dimension, speed, stress, fluid, drill) {
	...
	function CustomVeinDrillingF(title, output, spacing, separation, salt, id, sizem, sizeM, dimension, speed, stress, fluid) {
		event.recipes.createoreexcavation.vein(`Raw ${title}`, output)
		.placement(spacing, separation, salt)
		.veinSize(sizem, sizeM)
		.biomeWhitelist(`minecraft:is_${dimension}`)
	...
	function CustomVeinDrillingD(title, output, spacing, separation, salt, id, sizem, sizeM, dimension, speed, stress, drill) {
		event.recipes.createoreexcavation.vein(`Raw ${title}`, output)
		.placement(spacing, separation, salt)
		.veinSize(sizem, sizeM)
		.biomeWhitelist(`minecraft:is_${dimension}`)
	...
	CustomVeinDrillingFD('Aluminum', 'create_tank_defenses:raw_aluminum', 128, 8, 134567, 'raw_aluminum', 10, 30, 'overworld', 600, 256, '','diamond')
	CustomVeinDrillingFD('Titanium', 'create_tank_defenses:raw_titanium', 192, 64, 58768674, 'raw_titanium', 15, 40, 'overworld', 1200, 512, '','diamond')
